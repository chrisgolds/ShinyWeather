                 ...
                 observeEvent(input$getForecast, {
                   ...
                   output$timeOut <- renderUI({
                     ...
                     if (!(isTruthy(input$time))) {
                       ...
                       tags$div(sliderInput(inputId = session$ns("time"), slidetitle, min = 00, max = 21, value = hour, step = 3)
                                
                       )
                       
                     } else {
                       
                       tags$div(sliderInput(inputId = session$ns("time"), slidetitle, min = 00, max = 21, value = input$time, step = 3))
                   ...
                   output$dayOut <- renderUI({
                     ...
                     if (!(isTruthy(input$day))) {
                       ...
                       tags$div(selectInput(inputId = session$ns("day"), selecttitle, choices = formatted.dates))
                       
                     } else {
                       
                       tags$div(selectInput(inputId = session$ns("day"), selecttitle, choices = formatted.dates, selected = input$day))
                   ...
                   output$map <- renderLeaflet({
                     ...
                     output$pinnedWeather <- renderUI({
                       ...
                       actionButton(inputId = session$ns("pinnedWeatherHandle"), class = "btn btn-success", label = "Update location", width = "100%", disabled = TRUE)
